...
use Yproximite\Payum\Payzen\SignatureGenerator;
...
class SignatureGeneratorTest extends TestCase
{
     ...
     */
    public function testGenerate(string $expectedSignature, string $certificate, string $hashAlgorithm, array $fields): void
    {
        $signatureGenerator = new SignatureGenerator();

        $signature = $signatureGenerator->generate($fields, $certificate, $hashAlgorithm);

        static::assertEquals($expectedSignature, $signature);
    }

    public function provideTestGenerate(): \Generator
    {
