...
class NotifyActionTest extends GenericActionTest
{
     ...
     * @test
     */
    public function shouldUpdateModelIfNotificationValid(): void
    {
        ...
        $gatewayMock
            ->expects(static::once())
            ->method('execute')
            ->with(static::isInstanceOf('Payum\Core\Request\GetHttpRequest'))
            ->willReturnCallback(function (GetHttpRequest $request) {
                $request->request = $this->provideNotificationData();
            });

        $apiMock = $this->createApiMock();
        $apiMock->expects(static::once())->method('getSignature')->willReturn('833d20ce6b2c27dd054c289f7ba22a26aaf3350d');
        ...
        try {
            ...
            static::assertEquals($this->provideNotificationData(), (array) $model);

            static::assertSame(200, $reply->getStatusCode());
            static::assertSame('OK', $reply->getContent());
